...
/**
 * This sample demonstrates <b>how to manipulate the pages collection</b> within
 * a PDF document, to perform page data size calculations, additions, movements,
 * removals, extractions and splits of groups of pages.
 * 
...
public class PageManagementSample extends Sample {
		...
		try {
			...
			try {
				...
				throw new RuntimeException(
						mainFilePath + " file access error.", e);
			}
			...
			case PageDataSizeCalculation: {
				...
				System.out
						.println(" * full: page data size encompassing all its dependencies (like shared resources) -- this is the size of the page when extracted as a single-page document;");
				System.out
						.println(" * differential: additional page data size -- this is the extra-content that's not shared with previous pages;");
				System.out
						.println(" * incremental: data size of the page sublist encompassing all the previous pages and the current one.\n");
				...
				for (Page page : mainPages) {
					...
					long pageDifferentialDataSize = PageManager.getSize(page,
							visitedReferences);
					incrementalDataSize += pageDifferentialDataSize;

					System.out.println("Page " + (page.getIndex() + 1) + ": "
				...
				try {
					// Opening the source file...
					{
						...
						try {
							...
							throw new RuntimeException(sourceFilePath
									+ " file access error.", e);
						}
					}
					...
					// First page to add.
					int fromSourcePageIndex = promptPageChoice(
							"Select the start source page to add",
							sourcePagesCount);
					// Last page to add.
					int toSourcePageIndex = promptPageChoice(
							"Select the end source page to add",
							fromSourcePageIndex, sourcePagesCount) + 1;
					// Target position.
					int targetPageIndex = promptPageChoice(
							"Select the position where to insert the source pages",
							mainPagesCount + 1);

					// Add the chosen page range to the main document!
					new PageManager(mainDocument).add(targetPageIndex,
							sourcePages.subList(fromSourcePageIndex,
									toSourcePageIndex));
			...
			case PageMovement: {
				// First page to move.
				int fromSourcePageIndex = promptPageChoice(
						"Select the start page to move", mainPagesCount);
				// Last page to move.
				int toSourcePageIndex = promptPageChoice(
						"Select the end page to move", fromSourcePageIndex,
						mainPagesCount) + 1;
				// Target position.
				int targetPageIndex = promptPageChoice(
						"Select the position where to insert the pages",
						mainPagesCount + 1);

				// Move the chosen page range!
				new PageManager(mainDocument).move(fromSourcePageIndex,
						toSourcePageIndex, targetPageIndex);
			...
			case PageRemoval: {
				...
				int fromPageIndex = promptPageChoice(
						"Select the start page to remove", mainPagesCount);
				// Last page to remove.
				int toPageIndex = promptPageChoice(
						"Select the end page to remove", fromPageIndex,
						mainPagesCount) + 1;

				// Remove the chosen page range!
				new PageManager(mainDocument)
						.remove(fromPageIndex, toPageIndex);
			...
			case PageExtraction: {
				...
				int fromPageIndex = promptPageChoice("Select the start page",
						mainPagesCount);
				// Last page to extract.
				int toPageIndex = promptPageChoice("Select the end page",
						fromPageIndex, mainPagesCount) + 1;

				// Extract the chosen page range!
				Document targetDocument = new PageManager(mainDocument)
						.extract(fromPageIndex, toPageIndex);
			...
			case DocumentBurst: {
				...
				List<Document> splitDocuments = new PageManager(mainDocument)
						.split();
			...
			case DocumentSplitByPageIndex: {
				...
				try {
					splitCount = Integer
							.parseInt(promptChoice("Number of split positions: "));
				} catch (Exception e) {
					splitCount = 0;
				}
				...
				int[] splitIndexes = new int[splitCount];
				{
					...
					for (int index = 0; index < splitCount; index++) {
						int splitIndex = promptPageChoice("Position "
								+ (index + 1) + " of " + splitCount,
								prevSplitIndex + 1, mainPagesCount);
			...
			case DocumentSplitOnMaximumFileSize: {
				...
				long maxDataSize;
				{
					...
					do {
						try {
							kbMaxDataSize = Integer
									.parseInt(promptChoice("Max file size (KB): "));
						} catch (Exception e) {
							kbMaxDataSize = 0;
						}
	...
	private ActionEnum promptAction() {
		...
		for (ActionEnum action : actions) {
			options.put(Integer.toString(action.ordinal()),
	...
	private void serialize(File file, ActionEnum action, Integer index) {
		serialize(file, getClass().getSimpleName() + "_" + action.name()
				+ (index != null ? "." + index : ""), null, action.name(),
				"managing document pages", action.name());
	}
}