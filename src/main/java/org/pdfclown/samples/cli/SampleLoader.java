...
public class SampleLoader {
	...
	private static final String Properties_OutputPath = ClassName
			+ ".outputPath";
	...
	public static void main(String[] args) {
		...
		try {
			java.lang.Package pdfClownPackage = Class.forName(
					"org.pdfclown.Engine").getPackage();
			System.out.println(pdfClownPackage.getSpecificationTitle()
					+ " version " + pdfClownPackage.getImplementationVersion());
		} catch (Exception e) {
			throw new RuntimeException(
					"Unable to detect PDF Clown library version.", e);
		}
		...
		try {
			...
			throw new RuntimeException(
					"An exception occurred while loading the properties file (\""
							+ PropertiesFilePath + "\").", e);
		}

		run(new java.io.File(properties.getProperty(Properties_InputPath))
				.getAbsolutePath(),
				new java.io.File(properties.getProperty(Properties_OutputPath))
						.getAbsolutePath());
	...
	private static void run(String inputPath, String outputPath) {
		...
		while (true) {
			...
			List<Class<?>> samplePackageClasses = it.stefanochizzolini.reflex.Package
					.getClasses(samplePackageName,
							new String[] { it.stefanochizzolini.reflex.Class
									.getLocation(ClassName) } // Locations:
																// current
																// deployment
																// unit only.
					);
			Collections.sort(samplePackageClasses, new Comparator<Class<?>>() {
				@Override
			...
			for (Class<?> samplePackageClass : samplePackageClasses) {
				if (Sample.class.isAssignableFrom(samplePackageClass)
						&& !Modifier.isAbstract(samplePackageClass
								.getModifiers())) {
					sampleClasses.add(samplePackageClass);
					System.out.println("["
							+ sampleClasses.indexOf(samplePackageClass) + "] "
							+ samplePackageClass.getSimpleName());
				}
			}
			...
			try {
				...
				throw new RuntimeException(sampleClass.getName()
						+ " sample class has failed to instantiate.", e);
			}
			...
			try {
				...
				System.out
						.println("An exception happened while running the sample:");
				e.printStackTrace();
			}
		}
	}
